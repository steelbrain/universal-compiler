...
module.exports = (WatcherControl)->
  class H
    ...
    @FileInfo:(FullPath, RelativePath, Name)->
      ...
      ToReturn = Path: RelativePath + '/' + Name, Name:Name, Ext:Ext, Config: WatcherControl.FileTypes[Ext]
            ...
            Promises.push new Promise (ResolveFile)->
              FS.stat FullPath, (_, Stats)->
                if Stats.isDirectory()
                  RelativePath += '/' if RelativePath isnt ''
                  H.ScanDir(FullPath, RelativePath + Entry).then (Results)->
